...
SplineParser::SplineParser(const char* spline_file) {
    ...
    std::string path = "./resource/assignment8/" + std::string(spline_file);
...
void SplineParser::SaveBezier(ArgParser* args) {
    ...
    std::string path = "./resource/output/" + std::string(args->output_bezier_file);
...
void SplineParser::SaveBSpline(ArgParser* args) {
    ...
    std::string path = "./resource/output/" + std::string(args->output_bspline_file);
    FILE* file = fopen(path.c_str(), "w");
    assert(file != NULL);
    fprintf(file, "num_splines %d\n", getNumSplines());
    for (int i = 0; i < getNumSplines(); i++) {
...
void SplineParser::SaveTriangles(ArgParser* args) {
    ...
    std::string path = "./resource/output/" + std::string(args->output_file);
    FILE* file = fopen(path.c_str(), "w");
    assert(file != NULL);
    mesh.Output(file);
    fclose(file);
